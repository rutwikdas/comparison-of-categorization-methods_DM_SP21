...
dnn_s_model.fit(X_train,y_train)


# Check accuracy of our model on the test data
dnn_s_model.score(X_test, y_test)
...
import time
start = time.time()
cv_scores = cross_val_score(dnn_s_model, X_test, y_test, cv=10)
...
dnn_s_model_disp = plot_roc_curve(dnn_s_model, X_test, y_test, name= "ROC Curve of Deep NN on test data\n(Sigmoid, max iter=2000, SGD)")
plt.show()