...
dnn_r_model.fit(X_train,y_train)

# Check accuracy of our model on the test data
dnn_r_model.score(X_test, y_test)
...
import time
start = time.time()
cv_scores = cross_val_score(dnn_r_model, X_test, y_test, cv=10)
...
dnn_r_disp = plot_roc_curve(dnn_r_model, X_test, y_test, name= "ROC Curve of Deep NN on test data\n(ReLu, max iter=1400, SGD)")
plt.show()
