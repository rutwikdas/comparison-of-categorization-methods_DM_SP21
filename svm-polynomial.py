...
svc_p_model.fit(X_train,y_train)


# Check accuracy of our model on the test data
svc_p_model.score(X_test, y_test)
...
import time
start = time.time()
cv_scores = cross_val_score(svc_p_model, X, y, cv=10)
...
svc_p_disp = plot_roc_curve(svc_p_model, X_test, y_test, name= "ROC Curve of SVC on test data\n(Polynomial kernel, degree=1, max iter=200)")
plt.show()